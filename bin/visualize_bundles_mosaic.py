...
def _build_arg_parser():
    ...
    p.add_argument('--light_screenshot', action='store_true',
                   help='Keep only 3 views instead of 6 '
                        '[%(default)s].')
    p.add_argument('--no_first_column', action='store_true',
                   help='Don\'t show first column '
                        '[%(default)s].')
...
def main():
    ...
    if args.no_first_column:
        mosaic = Image.new('RGB', ((rows + 1) * height, (cols * width)))
    else:
